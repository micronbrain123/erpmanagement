...
export default function ReplaceRecord() {
  ...
  const [replaceRecords, setReplaceRecords] = useState([
    {
      ...
      itemName: 'Defective Laptop',
      replacedWith: 'New Laptop Computer',
      quantity: '1 Pc',
      originalRate: '45,000.00',
      replaceRate: '45,000.00',
      reason: 'Hardware Defect',
      replacedBy: 'John Doe',
      type: 'Warranty Replace'
    },
    {
      id: 2,
      date: '2025-08-14',
      ...
      itemName: 'Faulty Monitor',
      replacedWith: 'LED Monitor 24"',
      quantity: '1 Pc',
      originalRate: '18,500.00',
      replaceRate: '19,000.00',
  ...
  const filterTypes = ['All', 'Warranty Replace', 'Damage Replace', 'Quality Issue', 'Customer Request']
  ...
  const handleGetReport = () => {
    try {
      ...
      const filtered = replaceRecords.filter(record => {
  ...
  const handleSearch = () => {
    ...
    if (searchTerm.trim() === '') {
      ...
      const filtered = filteredRecords.filter(record =>
        record.replaceNo.toLowerCase().includes(searchTerm.toLowerCase()) ||
        record.customerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        record.itemName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        record.replacedWith.toLowerCase().includes(searchTerm.toLowerCase()) ||
        record.replacedBy.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Product Replace Record Report</title>
          <style>
        ...
        <body>
          <h1>Product Replace Record Report</h1>
            ...
            <tbody>
              ${filteredRecords.map(record => `
                <tr>
                  ...
                  <td>${record.replaceNo}</td>
                  <td>${record.customerName}</td>
                  <td>${record.itemName}</td>
                  <td>${record.replacedWith}</td>
                  <td>${record.quantity}</td>
                  <td class="text-right">${record.originalRate}</td>
                  <td class="text-right">${record.replaceRate}</td>
                  <td>${record.reason}</td>
  ...
  const handleExport = () => {
    const csvContent = [
      ['Date', 'Replace No', 'Customer Name', 'Item Name', 'Replaced With', 'Quantity', 'Original Rate', 'Replace Rate', 'Reason', 'Replaced By', 'Type'],
      ...filteredRecords.map(record => [
    ...
    a.href = url
    a.download = 'product_replace_record.csv'
  ...
  const handleDeleteRecord = (id) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      const updatedRecords = replaceRecords.filter(record => record.id !== id)
      setReplaceRecords(updatedRecords)
  ...
  return (
    <div className="p-4">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="p-4">
          <h2 className="font-medium text-lg mb-6">Product Replace Record</h2>
      ...
      {/* Replace Record Report */}
      {showTable && (
        <div className="bg-white rounded-lg shadow-sm border mt-6">
          <div className="p-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium">Product Replace Record Report</h3>
            ...
            {showSearch && (
              <div className="mb-4 flex gap-2">
                <input
                  type="text"
                  placeholder="Search by replace no, customer, item, reason, or type..."
                  value={searchTerm}
            ...
            <div className="overflow-x-auto">
              <div className="min-w-full">
                <div className="grid grid-cols-12 gap-2 text-xs font-semibold text-gray-700 border-b pb-2 mb-4">
                  ...
                  <div>Qty</div>
                  <div className="text-right">Orig. Rate</div>
                  <div className="text-right">New Rate</div>
                  <div>Reason</div>
                  <div>Replaced By</div>
                  <div>Type</div>
                  <div>Actions</div>
                ...
                {currentRecords.length > 0 ? (
                  currentRecords.map((record) => (
                    <div key={record.id} className="grid grid-cols-12 gap-2 text-xs py-2 border-b hover:bg-gray-50">
                      ...
                      <div className="font-medium text-teal-600">{record.replaceNo}</div>
                      ...
                      <div className="truncate" title={record.replacedWith}>{record.replacedWith}</div>
                      <div>{record.quantity}</div>
                      <div className="text-right">{record.originalRate} BDT</div>
                      <div className="text-right font-medium text-orange-600">{record.replaceRate} BDT</div>
                      <div className="truncate" title={record.reason}>{record.reason}</div>
                      <div>{record.replacedBy}</div>
                      <div>
                        <span className={`px-2 py-1 rounded text-xs ${
                          record.type === 'Warranty Replace' 
                            ? 'bg-green-100 text-green-800' 
                            : record.type === 'Damage Replace'
                            ? 'bg-red-100 text-red-800'
                            : record.type === 'Quality Issue'
