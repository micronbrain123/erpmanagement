...
export default function QuotationEntry() {
  ...
  const [filteredEntries, setFilteredEntries] = useState(quotationEntries)
  ...
  // Update filtered entries when quotation entries change
  useEffect(() => {
    handleSearch()
  }, [quotationEntries])
  ...
  useEffect(() => {
    calculateTotals()
  }, [quotationCart, formData.transportCost, formData.discountRs, formData.GSTRs])
  ...
  const handleAddToCart = () => {
    ...
    setQuotationCart(prev => [...prev, newCartItem])
  ...
  const calculateTotals = () => {
    const cartSubTotal = quotationCart.reduce((total, item) => {
  ...
  const handleSave = () => {
    ...
    setQuotationEntries(prev => [...prev, newEntry])
    setQuotationCart([])
    ...
    alert(`Quotation ${newEntry.quotationNo} saved successfully!`)
  }
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      setFilteredEntries(quotationEntries)
    } else {
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Quotation Entry List</title>
          <style>
        ...
        <body>
          <div class="header">
            ...
            <div class="report-title">Quotation Entry List</div>
  ...
  const handleExport = () => {
    const csvContent = [
      ['Fayullah Factory - Quotation Entry List'],
      [`Generated on: ${new Date().toLocaleString()}`],
      [`Total Records: ${filteredEntries.length}`],
      [],
      ['Quotation No', 'Customer', 'Date', 'Items', 'Total Amount', 'Status'],
      ...filteredEntries.map(entry => [
        entry.quotationNo,
        entry.customer,
        entry.date,
    ...
    a.href = url
    a.download = `quotation_entries_${formData.entryDate.replace(/\//g, '-')}.csv`
    ...
    <div className="p-4 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="bg-teal-600 text-white px-4 py-3 rounded-t-lg flex justify-between items-center">
          <h2 className="font-medium text-lg">Sales Quotation Entry</h2>
        ...
        <div className="p-6">
          ...
          {quotationCart.length > 0 && (
            <div className="mb-6">
              <h4 className="text-sm font-semibold text-gray-700 mb-3">Quotation Cart</h4>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <textarea 
                className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-teal-500"
                rows="4"
                placeholder="Enter quotation description or notes..."
      ...
      <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Quotation Entry List ({filteredEntries.length} records)</h3>
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b">
                  <th className="text-left py-3 px-2 text-xs font-semibold text-gray-700">Quotation No</th>
                ...
                {currentEntries.length > 0 ? (
                  currentEntries.map((entry) => (
                    <tr key={entry.id} className="border-b hover:bg-gray-50">
                      <td className="py-3 px-2 text-xs font-medium text-teal-600">{entry.quotationNo}</td>
                  ...
                  <tr>
                    <td colSpan="7" className="text-center py-8 text-gray-500">
                      {searchTerm ? 'No matching records found' : 'No quotation entries found'}
                    </td>
                  </tr>
                )}
              </tbody>
