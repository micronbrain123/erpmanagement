...
export default function AdvanceSupplierBalance() {
  const [filterType, setFilterType] = useState('All Creditor')
  ...
  // Sample supplier balance data
  const [supplierData] = useState([
    { 
      id: 1, 
      sl: 1, 
      creditorName: 'Arif Industries', 
      contactNo: '01777761111',
      address: 'Dhaka', 
      received: 25000.00, 
      payment: 30000.00, 
      balance: -5000.00
    ...
    { 
      id: 3, 
      sl: 3, 
      creditorName: 'Habib Trading', 
      contactNo: '12345678',
      address: 'House 12, Road 11, Sector 11, Uttara', 
      received: 45000.00, 
      payment: 45000.00, 
      balance: 0.00
    }
  ...
  const negativeBalance = filteredData.filter(item => item.balance < 0).length

  const handleGetReport = () => {
    console.log('Generating advance supplier balance report with filters:', {
      filterType,
      fromDate,
      toDate
    })
    ...
    if (filterType === 'Positive Balance') {
      filtered = supplierData.filter(item => item.balance > 0)
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      ...
      const filtered = supplierData.filter(entry =>
        entry.creditorName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Advance Creditor/Supplier Balance Report</title>
          <style>
        ...
        <body>
          <h1>Advance Creditor/Supplier Balance Report</h1>
  ...
  const handleExport = () => {
    const csvContent = [
      ['SL', 'Creditor Name', 'Contact No', 'Address', 'Received', 'Payment', 'Balance'],
      ...filteredData.map((entry) => [
        entry.sl,
        entry.creditorName,
        entry.contactNo,
    ...
    a.href = url
    a.download = 'advance_supplier_balance_report.csv'
    ...
    <div className="p-4">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="bg-teal-600 text-white px-4 py-3 rounded-t-lg">
          <h2 className="font-medium text-lg">Advance Creditor/Supplier Balance Report</h2>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Advance Creditor/Supplier Balance Report</h3>
          ...
          {showSearch && (
            <div className="mb-4 flex gap-2">
              <input
                type="text"
                placeholder="Search by creditor name, contact no, or address..."
                value={searchTerm}
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b bg-gray-50">
                  ...
                  <th className="text-left py-2 px-3 text-xs font-semibold text-gray-700 min-w-[150px]">Creditor Name</th>
                  ...
                  <>
                    {currentEntries.map((entry) => (
                      <tr key={entry.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-2 px-3 text-xs font-medium">{entry.creditorName}</td>
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                <span className="font-medium text-gray-600">Total Suppliers:</span>
                <div className="text-lg font-bold text-blue-600">{totalSuppliers}</div>
              </div>
              <div>
            ...
            <div className="grid grid-cols-3 gap-4 text-sm mt-2">
              ...
              <div>
                <span className="font-medium text-gray-600">Negative Balance:</span>
                <div className="text-lg font-bold text-red-600">{negativeBalance}</div>
              </div>
            </div>
          </div>
        </div>
