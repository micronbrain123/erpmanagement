...
export default function FixedAssetAccountsBalance() {
  const [filterType, setFilterType] = useState('All Fixed Asset Account')
  ...
  // Sample fixed asset accounts balance data with Indian context
  const [fixedAssetAccountsData] = useState([
    { 
      id: 1, 
      sl: 1, 
    ...
    { 
      ...
      assetAmount: 25000.00, 
      depreciationAmount: 12000.00, 
      balance: 93000.00
    },
    { 
      id: 4, 
      ...
      assetAmount: 15000.00, 
      depreciationAmount: 8000.00, 
      balance: 67000.00
    },
    { 
      id: 5, 
      ...
      assetAmount: 0.00, 
      depreciationAmount: 35000.00, 
      balance: 315000.00
    }
  ])

  const [filteredData, setFilteredData] = useState(fixedAssetAccountsData)

  // Filter options based on the screenshot
  const filterOptions = [
    'All Fixed Asset Account',
    'Building',
    'Machinery', 
    'Computer Equipment',
    'Furniture & Fixtures',
  ...
  const totalAssetAmount = filteredData.reduce((sum, item) => sum + item.assetAmount, 0)
  const totalDepreciationAmount = filteredData.reduce((sum, item) => sum + item.depreciationAmount, 0)
  const totalBalance = filteredData.reduce((sum, item) => sum + item.balance, 0)

  const handleGetReport = () => {
    console.log('Generating fixed asset accounts balance report with filters:', {
      filterType,
      fromDate,
      toDate
    })
    ...
    let filtered = fixedAssetAccountsData
    if (filterType !== 'All Fixed Asset Account') {
      filtered = fixedAssetAccountsData.filter(item => 
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      setFilteredData(fixedAssetAccountsData)
    } else {
      const filtered = fixedAssetAccountsData.filter(entry =>
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Fixed Asset Balance Report</title>
          <style>
        ...
        <body>
          <h1>Fixed Asset Balance Report</h1>
            ...
            <tbody>
              ${filteredData.map((entry) => `
                <tr>
                  ...
                  <td class="amount">₹${entry.assetAmount.toFixed(2)}</td>
                  <td class="amount">₹${entry.depreciationAmount.toFixed(2)}</td>
              ...
              <tr class="total-row">
                ...
                <td class="amount"><strong>₹${totalAssetAmount.toFixed(2)}</strong></td>
                <td class="amount"><strong>₹${totalDepreciationAmount.toFixed(2)}</strong></td>
  ...
  const handleExport = () => {
    const csvContent = [
      ['SL', 'Account Name', 'Opening Balance', 'Asset Amount', 'Depreciation Amount', 'Balance'],
      ...filteredData.map((entry) => [
        entry.sl,
        entry.accountName,
        entry.openingBalance.toFixed(2),
        entry.assetAmount.toFixed(2),
        entry.depreciationAmount.toFixed(2),
        entry.balance.toFixed(2)
      ]),
      ['Grand Total', '', totalOpeningBalance.toFixed(2), totalAssetAmount.toFixed(2), totalDepreciationAmount.toFixed(2), totalBalance.toFixed(2)]
    ...
    a.href = url
    a.download = 'fixed_asset_balance_report.csv'
    ...
    <div className="p-4">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="bg-teal-600 text-white px-4 py-3 rounded-t-lg">
          <h2 className="font-medium text-lg">Fixed Asset Balance Report</h2>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Fixed Asset Balance Report</h3>
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b bg-gray-50">
                  ...
                  <th className="text-right py-2 px-3 text-xs font-semibold text-gray-700">Asset Amount</th>
                  <th className="text-right py-2 px-3 text-xs font-semibold text-gray-700">Depreciation Amount</th>
                ...
                {currentEntries.length > 0 ? (
                  <>
                    {currentEntries.map((entry) => (
                      <tr key={entry.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-2 px-3 text-xs text-right">
                          <span className={entry.assetAmount > 0 ? 'text-green-600' : 'text-gray-600'}>
                            ₹{entry.assetAmount.toFixed(2)}
                          </span>
                        </td>
                        <td className="py-2 px-3 text-xs text-right">
                          <span className={entry.depreciationAmount > 0 ? 'text-red-600' : 'text-gray-600'}>
                            ₹{entry.depreciationAmount.toFixed(2)}
                          </span>
                        </td>
                        <td className="py-2 px-3 text-xs text-right">
                    ...
                    <tr className="border-t-2 border-gray-400 bg-gray-100">
                      ...
                      <td className="py-2 px-3 text-xs text-right font-bold">₹{totalAssetAmount.toFixed(2)}</td>
                      <td className="py-2 px-3 text-xs text-right font-bold">₹{totalDepreciationAmount.toFixed(2)}</td>
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                <span className="font-medium text-gray-600">Asset Addition:</span>
                <div className="text-lg font-bold text-green-600">₹{totalAssetAmount.toFixed(2)}</div>
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mt-2">
              <div>
                <span className="font-medium text-gray-600">Total Depreciation:</span>
                <div className="text-lg font-bold text-red-600">₹{totalDepreciationAmount.toFixed(2)}</div>
              </div>
              <div>
                <span className="font-medium text-gray-600">Net Book Value:</span>
                <div className="text-lg font-bold text-blue-600">₹{(totalOpeningBalance + totalAssetAmount - totalDepreciationAmount).toFixed(2)}</div>
              </div>
            </div>
          </div>
        </div>
