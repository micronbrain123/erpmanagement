...
export default function IndirectIncomeAccountLedger() {
  ...
  // Sample indirect income account ledger data with Indian context
  const [indirectIncomeLedgerData] = useState([
    { 
      id: 1, 
      sl: 1, 
      date: '21/08/2025',
      particulars: 'Interest Income - Fixed Deposit SBI', 
      voucherNo: 'II001',
      debit: 0.00, 
      credit: 15000.00, 
      balance: 15000.00
    ...
    { 
      ...
      sl: 2, 
      date: '20/08/2025',
      particulars: 'Dividend Income - Mutual Funds', 
      voucherNo: 'II002',
      debit: 0.00, 
      credit: 8500.00, 
      balance: 23500.00
    ...
    { 
      ...
      sl: 4, 
      date: '18/08/2025',
      particulars: 'Commission Received - Agency', 
      voucherNo: 'II004',
      debit: 0.00, 
      credit: 12000.00, 
      balance: 80500.00
    ...
    { 
      ...
      sl: 5, 
      date: '17/08/2025',
      particulars: 'Miscellaneous Income - Scrap Sales', 
      voucherNo: 'II005',
      debit: 0.00, 
      credit: 3500.00, 
      balance: 84000.00
    ...
    { 
      ...
      sl: 7, 
      date: '15/08/2025',
      particulars: 'Foreign Exchange Gain', 
      voucherNo: 'II007',
      debit: 0.00, 
      credit: 7500.00, 
      balance: 116500.00
  ...
  const [filteredData, setFilteredData] = useState(indirectIncomeLedgerData)

  // Filter options for indirect income account ledger
  const filterOptions = [
    'Search Account',
    'Interest Income',
    'Dividend Income', 
  ...
  const handleGetReport = () => {
    console.log('Generating indirect income account ledger with filters:', {
      filterType,
      fromDate,
      toDate
    })
    ...
    let filtered = indirectIncomeLedgerData
    ...
    if (filterType !== 'Search Account') {
      if (filterType === 'Interest Income') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('interest')
        )
      } else if (filterType === 'Dividend Income') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('dividend')
        )
      } else if (filterType === 'Rental Income') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('rental') ||
          item.particulars.toLowerCase().includes('rent')
        )
      } else if (filterType === 'Commission Income') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('commission')
        )
      } else if (filterType === 'Miscellaneous Income') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('miscellaneous')
        )
      } else if (filterType === 'Foreign Exchange Gain') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('foreign exchange') ||
          item.particulars.toLowerCase().includes('forex')
        )
      } else if (filterType === 'Tax Refunds') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('tax refund') ||
          item.particulars.toLowerCase().includes('refund')
        )
      }
    }
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      setFilteredData(indirectIncomeLedgerData)
    } else {
      const filtered = indirectIncomeLedgerData.filter(entry =>
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Indirect Income Balance Ledger</title>
          <style>
        ...
        <body>
          <h1>Indirect Income Balance Ledger</h1>
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = 'indirect_income_balance_ledger.csv'
    ...
    <div className="p-4">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="bg-teal-600 text-white px-4 py-3 rounded-t-lg">
          <h2 className="font-medium text-lg">Indirect Income Balance Ledger</h2>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Indirect Income Balance Ledger</h3>
          ...
          {showSearch && (
            <div className="mb-4 flex gap-2">
              <input
                type="text"
                placeholder="Search by income description or voucher number..."
                value={searchTerm}
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                <span className="font-medium text-gray-600">Total Sources:</span>
              ...
              <div>
                <span className="font-medium text-gray-600">Adjustments:</span>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mt-2">
              <div>
                <span className="font-medium text-gray-600">Average per Source:</span>
              ...
              <div>
                ...
                <div className="text-lg font-bold text-orange-600">
                  {totalCredit > 0 ? ((netBalance / totalCredit) * 100).toFixed(2) : '0.00'}%
                </div>
              </div>
            </div>
          </div>
