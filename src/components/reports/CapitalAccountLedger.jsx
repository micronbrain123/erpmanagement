...
export default function CapitalAccountLedger() {
  ...
  // Sample capital account ledger data with Indian context
  const [capitalAccountLedgerData] = useState([
    { 
      id: 1, 
      sl: 1, 
      date: '21/08/2025',
      particulars: 'Initial Capital Contribution - Partner A', 
      voucherNo: 'CA001',
      debit: 0.00, 
      credit: 1000000.00, 
      balance: 1000000.00
    ...
    { 
      ...
      sl: 2, 
      date: '20/08/2025',
      particulars: 'Additional Capital Investment - Partner B', 
      voucherNo: 'CA002',
      debit: 0.00, 
      credit: 500000.00, 
      balance: 1500000.00
    ...
    { 
      ...
      sl: 3, 
      date: '19/08/2025',
      particulars: 'Capital Withdrawal - Partner A', 
      voucherNo: 'CA003',
      debit: 50000.00, 
      credit: 0.00, 
      balance: 1450000.00
    ...
    { 
      ...
      sl: 4, 
      date: '18/08/2025',
      particulars: 'Share Premium Receipt', 
      voucherNo: 'CA004',
      debit: 0.00, 
      credit: 200000.00, 
      balance: 1650000.00
    ...
    { 
      ...
      sl: 5, 
      date: '17/08/2025',
      particulars: 'Retained Earnings Transfer', 
      voucherNo: 'CA005',
      debit: 0.00, 
      credit: 150000.00, 
      balance: 1800000.00
    ...
    { 
      ...
      sl: 7, 
      date: '15/08/2025',
      particulars: 'Capital Reserve Creation', 
      voucherNo: 'CA007',
      debit: 0.00, 
      credit: 100000.00, 
      balance: 1825000.00
    ...
    { 
      ...
      sl: 8, 
      date: '14/08/2025',
      particulars: 'Partner Salary Advance', 
      voucherNo: 'CA008',
      debit: 25000.00, 
      credit: 0.00, 
      balance: 1800000.00
  ...
  const [filteredData, setFilteredData] = useState(capitalAccountLedgerData)

  // Filter options for capital account ledger
  const filterOptions = [
    'Search Account',
    'Share Capital',
    'Paid-up Capital', 
  ...
  const handleGetReport = () => {
    console.log('Generating capital account ledger with filters:', {
      filterType,
      fromDate,
      toDate
    })
    ...
    let filtered = capitalAccountLedgerData
    ...
    if (filterType !== 'Search Account') {
      if (filterType === 'Share Capital') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('share') ||
          item.particulars.toLowerCase().includes('capital contribution')
        )
      } else if (filterType === 'Additional Capital') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('additional') ||
          item.particulars.toLowerCase().includes('investment')
        )
      } else if (filterType === 'Capital Withdrawals') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('withdrawal') ||
          item.particulars.toLowerCase().includes('advance')
        )
      } else if (filterType === 'Share Premium') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('premium')
        )
      } else if (filterType === 'Retained Earnings') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('retained') ||
          item.particulars.toLowerCase().includes('earnings')
        )
      } else if (filterType === 'Capital Reserves') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('reserve')
        )
      } else if (filterType === 'Dividend Payments') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('dividend')
        )
      } else if (filterType === 'Partner Capital') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('partner')
        )
      }
    }
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      setFilteredData(capitalAccountLedgerData)
    } else {
      const filtered = capitalAccountLedgerData.filter(entry =>
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Capital Balance Ledger</title>
          <style>
  ...
  const handleExport = () => {
    const csvContent = [
      ...
      ['Grand Total', '', '', '', totalDebit.toFixed(2), totalCredit.toFixed(2), netBalance.toFixed(2)]
    ...
    a.href = url
    a.download = 'capital_balance_ledger.csv'
    ...
    <div className="p-4">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="bg-teal-600 text-white px-4 py-3 rounded-t-lg">
          <h2 className="font-medium text-lg">Capital Balance Ledger</h2>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Capital Balance Ledger</h3>
          ...
          {showSearch && (
            <div className="mb-4 flex gap-2">
              <input
                type="text"
                placeholder="Search by capital transaction or voucher number..."
                value={searchTerm}
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                <span className="font-medium text-gray-600">Capital Invested:</span>
                <div className="text-lg font-bold text-blue-600">₹{totalCredit.toFixed(2)}</div>
              </div>
              <div>
                <span className="font-medium text-gray-600">Capital Withdrawn:</span>
                <div className="text-lg font-bold text-red-600">₹{totalDebit.toFixed(2)}</div>
              </div>
              <div>
                <span className="font-medium text-gray-600">Net Capital:</span>
                <div className="text-lg font-bold text-green-600">₹{netBalance.toFixed(2)}</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm mt-2">
              <div>
                <span className="font-medium text-gray-600">Average Transaction:</span>
                <div className="text-lg font-bold text-purple-600">₹{totalEntries > 0 ? ((totalCredit + totalDebit) / totalEntries).toFixed(2) : '0.00'}</div>
              </div>
              <div>
