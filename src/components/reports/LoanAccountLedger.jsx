...
export default function LoanAccountLedger() {
  ...
  // Sample loan account ledger data with Indian context
  const [loanAccountLedgerData] = useState([
    { 
      id: 1, 
      sl: 1, 
      date: '21/08/2025',
      particulars: 'Bank Loan - HDFC Term Loan', 
      voucherNo: 'LN001',
      debit: 0.00, 
      credit: 2500000.00, 
      balance: -2500000.00
    ...
    { 
      ...
      sl: 2, 
      date: '20/08/2025',
      particulars: 'Loan Repayment - Principal', 
      voucherNo: 'LN002',
      debit: 25000.00, 
      credit: 0.00, 
      balance: -2475000.00
    ...
    { 
      ...
      sl: 3, 
      date: '20/08/2025',
      particulars: 'Interest on Loan - HDFC', 
      voucherNo: 'LN003',
      debit: 18750.00, 
      credit: 0.00, 
      balance: -2456250.00
    ...
    { 
      ...
      sl: 4, 
      date: '19/08/2025',
      particulars: 'Personal Loan - Staff Advance', 
      voucherNo: 'LN004',
      debit: 0.00, 
      credit: 50000.00, 
      balance: -2506250.00
    ...
    { 
      ...
      sl: 5, 
      date: '18/08/2025',
      particulars: 'Vehicle Loan EMI Payment', 
      voucherNo: 'LN005',
      debit: 12500.00, 
      credit: 0.00, 
      balance: -2493750.00
  ...
  const [filteredData, setFilteredData] = useState(loanAccountLedgerData)

  // Filter options for loan account ledger based on your screenshot
  const filterOptions = [
    'Search Account',
    'Bank Loans',
    'Term Loans', 
  ...
  const netBalance = totalCredit - totalDebit // For loans, credit balance means liability

  const handleGetReport = () => {
    console.log('Generating loan account ledger with filters:', {
      filterType,
      fromDate,
      toDate
    })
    ...
    let filtered = loanAccountLedgerData
    ...
    if (filterType !== 'Search Account') {
      if (filterType === 'Bank Loans') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('bank') ||
          item.particulars.toLowerCase().includes('hdfc') ||
          item.particulars.toLowerCase().includes('sbi')
        )
      } else if (filterType === 'Term Loans') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('term loan')
        )
      } else if (filterType === 'Personal Loans') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('personal') ||
          item.particulars.toLowerCase().includes('staff advance')
        )
      } else if (filterType === 'Vehicle Loans') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('vehicle') ||
          item.particulars.toLowerCase().includes('car') ||
          item.particulars.toLowerCase().includes('auto')
        )
      } else if (filterType === 'Loan Interest') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('interest')
        )
      } else if (filterType === 'Loan Repayments') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('repayment') ||
          item.particulars.toLowerCase().includes('emi')
        )
      }
    }
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      setFilteredData(loanAccountLedgerData)
    } else {
      const filtered = loanAccountLedgerData.filter(entry =>
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = 'loan_account_ledger.csv'
    ...
    <div className="p-4">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="bg-teal-600 text-white px-4 py-3 rounded-t-lg">
          <h2 className="font-medium text-lg">Loan Account Ledger</h2>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Loan Account Ledger</h3>
          ...
          {showSearch && (
            <div className="mb-4 flex gap-2">
              <input
                type="text"
                placeholder="Search by loan description or voucher number..."
                value={searchTerm}
                  ...
                  <>
                    {currentEntries.map((entry) => (
                      <tr key={entry.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-2 px-3 text-xs text-right">
                          <span className={entry.balance < 0 ? 'text-red-600 font-medium' : 'text-blue-600 font-medium'}>
                            ₹{entry.balance.toFixed(2)}
                          </span>
                        </td>
                      </tr>
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                <span className="font-medium text-gray-600">Total Loans:</span>
              ...
              <div>
                <span className="font-medium text-gray-600">Total Repaid:</span>
              ...
              <div>
                ...
                <div className="text-lg font-bold text-orange-600">₹{Math.abs(netBalance).toFixed(2)}</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm mt-2">
              <div>
                <span className="font-medium text-gray-600">Average Loan Size:</span>
                <div className="text-lg font-bold text-purple-600">₹{totalEntries > 0 ? (totalCredit / totalEntries).toFixed(2) : '0.00'}</div>
              </div>
              <div>
                <span className="font-medium text-gray-600">Repayment Rate:</span>
                <div className="text-lg font-bold text-green-600">{totalCredit > 0 ? ((totalDebit / totalCredit) * 100).toFixed(2) : '0.00'}%</div>
              </div>
            </div>
          </div>
        </div>
