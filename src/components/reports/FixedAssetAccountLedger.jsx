...
export default function FixedAssetAccountLedger() {
  ...
  // Sample fixed asset account ledger data with Indian context
  const [fixedAssetLedgerData] = useState([
    { 
      id: 1, 
      sl: 1, 
      date: '21/08/2025',
      particulars: 'Purchase of Office Building - Mumbai', 
      voucherNo: 'FA001',
      debit: 5000000.00, 
      credit: 0.00, 
      balance: 5000000.00
    ...
    { 
      ...
      sl: 2, 
      date: '20/08/2025',
      particulars: 'Computer & IT Equipment Purchase', 
      voucherNo: 'FA002',
      debit: 350000.00, 
      credit: 0.00, 
      balance: 5350000.00
  ...
  const [filteredData, setFilteredData] = useState(fixedAssetLedgerData)

  // Filter options for fixed asset account ledger
  const filterOptions = [
    'Search Account',
    'Building & Property',
    'Machinery & Equipment', 
  ...
  const handleGetReport = () => {
    console.log('Generating fixed asset account ledger with filters:', {
      filterType,
      fromDate,
      toDate
    })
    ...
    let filtered = fixedAssetLedgerData
    ...
    if (filterType !== 'Search Account') {
      if (filterType === 'Building & Property') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('building') ||
          item.particulars.toLowerCase().includes('property')
        )
      } else if (filterType === 'Computer & IT Assets') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('computer') ||
          item.particulars.toLowerCase().includes('it')
        )
      } else if (filterType === 'Depreciation Accounts') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('depreciation')
        )
      }
    }
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      setFilteredData(fixedAssetLedgerData)
    } else {
      const filtered = fixedAssetLedgerData.filter(entry =>
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Fixed Asset Balance Ledger</title>
          <style>
        ...
        <body>
          <h1>Fixed Asset Balance Ledger</h1>
  ...
  const handleExport = () => {
    ...
    a.href = url
    a.download = 'fixed_asset_balance_ledger.csv'
    ...
    <div className="p-4">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="bg-teal-600 text-white px-4 py-3 rounded-t-lg">
          <h2 className="font-medium text-lg">Fixed Asset Balance Ledger</h2>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Fixed Asset Balance Ledger</h3>
          ...
          {showSearch && (
            <div className="mb-4 flex gap-2">
              <input
                type="text"
                placeholder="Search by asset description or voucher number..."
                value={searchTerm}
                    ...
                    {currentEntries.map((entry) => (
                      <tr key={entry.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-2 px-3 text-xs text-right">
                          <span className={entry.debit > 0 ? 'text-blue-600 font-medium' : 'text-gray-600'}>
                            ₹{entry.debit.toFixed(2)}
                          </span>
                        </td>
                        <td className="py-2 px-3 text-xs text-right">
                          <span className={entry.credit > 0 ? 'text-red-600' : 'text-gray-600'}>
                            ₹{entry.credit.toFixed(2)}
                          </span>
                        </td>
                        <td className="py-2 px-3 text-xs text-right">
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                <span className="font-medium text-gray-600">Gross Asset Value:</span>
                <div className="text-lg font-bold text-blue-600">₹{totalDebit.toFixed(2)}</div>
              </div>
              <div>
                <span className="font-medium text-gray-600">Total Depreciation:</span>
                <div className="text-lg font-bold text-red-600">₹{totalCredit.toFixed(2)}</div>
              </div>
              <div>
                <span className="font-medium text-gray-600">Net Asset Value:</span>
                <div className="text-lg font-bold text-green-600">₹{netBalance.toFixed(2)}</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm mt-2">
              <div>
                <span className="font-medium text-gray-600">Average Asset Value:</span>
                <div className="text-lg font-bold text-purple-600">₹{totalEntries > 0 ? (netBalance / totalEntries).toFixed(2) : '0.00'}</div>
              </div>
              <div>
