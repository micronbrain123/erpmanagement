...
export default function PurchaseAccountLedger() {
  ...
  // Sample purchase account ledger data with Indian context
  const [purchaseAccountLedgerData] = useState([
    { 
      id: 1, 
      sl: 1, 
      date: '21/08/2025',
      particulars: 'Raw Material Purchase - M/s Steel Industries Mumbai', 
      voucherNo: 'PUR001',
    ...
    { 
      ...
      sl: 2, 
      date: '20/08/2025',
      particulars: 'Inventory Purchase - Textile Suppliers Delhi', 
      voucherNo: 'PUR002',
      voucherType: 'Purchase',
      debit: 325000.00, 
      credit: 0.00, 
    ...
    { 
      ...
      sl: 3, 
      date: '19/08/2025',
      particulars: 'Purchase Return - Defective Items Chennai', 
      voucherNo: 'PR001',
    ...
    { 
      ...
      sl: 4, 
      date: '18/08/2025',
      particulars: 'Import Purchase - Electronic Components Kolkata Port', 
      voucherNo: 'IMP001',
      voucherType: 'Import Purchase',
      debit: 785000.00, 
      credit: 0.00, 
      balance: 1560000.00
    },
    { 
      ...
      sl: 5, 
      date: '17/08/2025',
      particulars: 'Bulk Purchase - Chemical Suppliers Bangalore', 
      voucherNo: 'BP001',
      voucherType: 'Bulk Purchase',
      debit: 625000.00, 
      credit: 0.00, 
      balance: 2185000.00
    },
    { 
      ...
      sl: 6, 
      date: '16/08/2025',
      particulars: 'Equipment Purchase - Machinery Dealers Hyderabad', 
      voucherNo: 'EP001',
      voucherType: 'Equipment Purchase',
      debit: 950000.00, 
      credit: 0.00, 
      balance: 3135000.00
    },
    { 
      ...
      sl: 7, 
      date: '15/08/2025',
      particulars: 'Local Purchase - Office Supplies Pune', 
      voucherNo: 'LP001',
    ...
    { 
      ...
      sl: 8, 
      date: '14/08/2025',
      particulars: 'Purchase Discount Received - Early Payment Jaipur', 
      voucherNo: 'PD001',
    ...
    { 
      ...
      sl: 9, 
      date: '13/08/2025',
      particulars: 'Contract Purchase - Government Tender Ahmedabad', 
      voucherNo: 'CP001',
      voucherType: 'Contract Purchase',
      debit: 450000.00, 
      credit: 0.00, 
      balance: 3605000.00
    },
    { 
      ...
      sl: 10, 
      date: '12/08/2025',
      particulars: 'Wholesale Purchase - Trading Company Lucknow', 
      voucherNo: 'WP001',
      voucherType: 'Wholesale Purchase',
      debit: 375000.00, 
      credit: 0.00, 
      balance: 3980000.00
    }
  ...
  const [filteredData, setFilteredData] = useState(purchaseAccountLedgerData)

  // Filter options for purchase account ledger
  const filterOptions = [
    'Search Account',
    'Purchase',
  ...
  const handleGetReport = () => {
    console.log('Generating purchase account ledger with filters:', {
      filterType,
      fromDate,
      toDate
    })
    ...
    let filtered = purchaseAccountLedgerData
    ...
    if (filterType !== 'Search Account') {
      if (filterType === 'Purchase') {
        filtered = filtered.filter(item => 
          item.voucherType.toLowerCase().includes('purchase') &&
          !item.voucherType.toLowerCase().includes('return')
        )
      } else if (filterType === 'Raw Material Purchase') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('raw material')
        )
      } else if (filterType === 'Inventory Purchase') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('inventory')
        )
      } else if (filterType === 'Import Purchase') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('import') ||
          item.voucherType.toLowerCase().includes('import')
        )
      } else if (filterType === 'Bulk Purchase') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('bulk') ||
          item.voucherType.toLowerCase().includes('bulk')
        )
      } else if (filterType === 'Equipment Purchase') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('equipment') ||
          item.voucherType.toLowerCase().includes('equipment')
        )
      } else if (filterType === 'Local Purchase') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('local') ||
          item.voucherType.toLowerCase().includes('local')
        )
      } else if (filterType === 'Contract Purchase') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('contract') ||
          item.voucherType.toLowerCase().includes('contract')
        )
      } else if (filterType === 'Wholesale Purchase') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('wholesale') ||
          item.voucherType.toLowerCase().includes('wholesale')
        )
      } else if (filterType === 'Purchase Returns') {
        filtered = filtered.filter(item => 
          item.particulars.toLowerCase().includes('return') ||
          item.voucherType.toLowerCase().includes('return')
        )
      } else if (filterType === 'Purchase Discounts') {
        filtered = filtered.filter(item => 
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      setFilteredData(purchaseAccountLedgerData)
    } else {
      const filtered = purchaseAccountLedgerData.filter(entry =>
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Purchase Account Ledger</title>
          <style>
          ...
          <table>
            ...
            <tbody>
              <tr class="opening-balance">
                ...
                <td class="amount"><strong>₹3,510,120.00</strong></td>
  ...
  const handleExport = () => {
    const csvContent = [
      ...
      ['Opening Balance', '', '', '', '', '3510120.00', '0.00', ''],
      ...filteredData.map((entry) => [
        entry.sl,
        entry.date,
        entry.particulars,
    ...
    a.href = url
    a.download = 'purchase_account_ledger.csv'
    ...
    <div className="p-4">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="bg-teal-600 text-white px-4 py-3 rounded-t-lg">
          <h2 className="font-medium text-lg">Purchase Account Ledger</h2>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Purchase Account Ledger</h3>
          ...
          {showSearch && (
            <div className="mb-4 flex gap-2">
              <input
                type="text"
                placeholder="Search by purchase transaction, voucher number or type..."
                value={searchTerm}
            ...
            <table className="min-w-full">
              ...
              <tbody>
                ...
                <tr className="border-b bg-green-50">
                  ...
                  <td className="py-2 px-3 text-xs text-right font-bold">₹3,510,120.00</td>
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                <span className="font-medium text-gray-600">Returns/Discounts:</span>
                <div className="text-lg font-bold text-green-600">₹{totalCredit.toFixed(2)}</div>
              </div>
              <div>
                <span className="font-medium text-gray-600">Net Purchases:</span>
                <div className="text-lg font-bold text-blue-600">₹{netBalance.toFixed(2)}</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm mt-2">
              <div>
                <span className="font-medium text-gray-600">Average Purchase Value:</span>
