...
export default function IndirectIncomeAccountsBalance() {
  const [filterType, setFilterType] = useState('All Income Account')
  ...
  // Sample income accounts balance data
  const [incomeAccountsData] = useState([
    { 
      id: 1, 
      sl: 1, 
  ...
  const [filteredData, setFilteredData] = useState(incomeAccountsData)
  ...
  const totalClosingBalance = filteredData.reduce((sum, item) => sum + item.closingBalance, 0)
  const positiveBalance = filteredData.filter(item => item.closingBalance > 0).length
  const zeroBalance = filteredData.filter(item => item.closingBalance === 0).length

  const handleGetReport = () => {
    console.log('Generating indirect income accounts balance report with filters:', {
      filterType,
      fromDate,
      toDate
    })
    ...
    let filtered = incomeAccountsData
    if (filterType !== 'All Income Account') {
      filtered = incomeAccountsData.filter(item => 
  ...
  const handleSearch = () => {
    if (searchTerm.trim() === '') {
      setFilteredData(incomeAccountsData)
    } else {
      const filtered = incomeAccountsData.filter(entry =>
  ...
  const handlePrint = () => {
    const printContent = `
      <html>
        <head>
          <title>Indirect Income Balance Report</title>
          <style>
        ...
        <body>
          <h1>Indirect Income Balance Report</h1>
            ...
            <tbody>
              ${filteredData.map((entry) => `
                <tr>
                  ...
                  <td class="amount">${entry.closingBalance.toFixed(2)}</td>
                </tr>
              `).join('')}
              <tr class="total-row">
                ...
                <td class="amount"><strong>${totalClosingBalance.toFixed(2)}</strong></td>
              </tr>
            </tbody>
          </table>
        </body>
  ...
  const handleExport = () => {
    const csvContent = [
      ['SL', 'Account Name', 'Account Code', 'Opening Balance', 'Debit', 'Credit', 'Closing Balance'],
      ...filteredData.map((entry) => [
        entry.sl,
        entry.accountName,
        entry.accountCode,
      ...
      ['Grand Total', '', '', totalOpeningBalance.toFixed(2), totalDebit.toFixed(2), totalCredit.toFixed(2), totalClosingBalance.toFixed(2)]
    ...
    a.href = url
    a.download = 'indirect_income_balance_report.csv'
    ...
    <div className="p-4">
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="bg-teal-600 text-white px-4 py-3 rounded-t-lg">
          <h2 className="font-medium text-lg">Indirect Income Balance Report</h2>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="p-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-medium">Indirect Income Balance Report</h3>
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b bg-gray-50">
                  ...
                  <th className="text-center py-2 px-3 text-xs font-semibold text-gray-700">Account Code</th>
                  ...
                  <th className="text-right py-2 px-3 text-xs font-semibold text-gray-700">Closing Balance</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <>
                    {currentEntries.map((entry) => (
                      <tr key={entry.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-2 px-3 text-xs text-center font-mono">{entry.accountCode}</td>
                        ...
                        <td className="py-2 px-3 text-xs text-right">
                          <span className={
                            entry.closingBalance > 0 ? 'text-green-600 font-medium' :
                            entry.closingBalance < 0 ? 'text-red-600 font-medium' :
                            'text-gray-600'
                          }>
                            ₹{entry.closingBalance.toFixed(2)}
                          </span>
                    ...
                    <tr className="border-t-2 border-gray-400 bg-gray-100">
                      ...
                      <td className="py-2 px-3 text-xs text-right font-bold">₹{totalClosingBalance.toFixed(2)}</td>
                    </tr>
                  </>
                ) : (
                  <tr>
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded border">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <div className={`text-lg font-bold ${totalClosingBalance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mt-2">
              ...
              <div>
                ...
                <div className="text-lg font-bold text-green-600">₹{(totalCredit - totalDebit).toFixed(2)}</div>
              </div>
            </div>
          </div>
        </div>
